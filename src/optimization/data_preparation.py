...
class DataPreparation:
    ...
    def calculate_adaptive_parameters(data, future_periods, is_intraday):
        ...
        if is_intraday:
            ...
            initial_str = f"{initial_periods // 24} days" if initial_periods >= 24 else f"{initial_periods} hours"
            period_str = f"{period_periods // 24} days" if period_periods >= 24 else f"{period_periods} hours"
            horizon_str = f"{horizon_periods // 24} days" if horizon_periods >= 24 else f"{horizon_periods} hours"
        else:
